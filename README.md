...
8. 调用 `POST /generate/api` 输入接口需求，生成 Controller / Service / Schema / Test 草稿。
9. 调用 `POST /generate/test` 输入文件或函数名，生成 pytest 单元测试草稿。
10. 调用 `GET /runs?repository_id=<id>` 查看刚才所有 Agent 调用历史。
