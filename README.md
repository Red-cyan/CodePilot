...
- 支持基于真实模型生成架构分析、Bug 分析、PR Review、README 草稿、API 实现草稿和单元测试草稿。
...
4. 调用 `POST /chat` 提问，例如：“这个项目的仓库导入逻辑是怎么实现的？”
5. 调用 `POST /analyze/architecture` 查看架构分析报告。
6. 调用 `POST /analyze/bug` 输入错误日志，查看相关代码定位结果。
7. 调用 `POST /review` 粘贴 git diff，生成 PR Review。
8. 调用 `POST /generate/readme` 生成 README 草稿。
9. 调用 `POST /generate/api` 输入接口需求，生成 Controller / Service / Schema / Test 草稿。
10. 调用 `POST /generate/test` 输入文件或函数名，生成 pytest 单元测试草稿。
11. 调用 `GET /runs?repository_id=<id>` 查看刚才所有 Agent 调用历史。
